...
// Lights
const bool Lights = 0;          // Enable lights
const bool LightsOnStartup = 0; // Turn on lights when calibrating
...
#define LightDPin 11            // Lights' Digital pin
...
void manual_calibration(int times = 100) {
  ...
  if(generalOrQTR) {
    ...
    if(Cali) {
      ...
      for (int j = 0; j < 4; j++) generalMaxCali[j] = 900;
      for (int j = 0; j < 4; j++) generalMinCali[j] = 630;
      delay(2000);
      modee = 1;
    }
...
void linetracingMode2() {
  ...
  if(pos < 1900 && pos > 1100) ++reli; // In-between the lines (currently stable).
  else reli = 0;
  // if(reli < 20) {
  //   initSpeed = 195;
  //   Kp = 55;
  ...
  else if(specialMode == 0 || (specialMode && error > -2.98 && error < 2.98)) {
    speCD = 500;
    specialMode = 0;
    initSpeed = 230;
    Kp = 20;
  ...
  if(speCD) speCD--;
  if(sumError < 0.1 && speCD == 0) {
    specialMode = 1;
  }
  if(specialMode) digitalWrite(LightDPin, HIGH);
